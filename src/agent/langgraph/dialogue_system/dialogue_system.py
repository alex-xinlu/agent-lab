...
def debug_node(state: SearchState) -> SearchState:
    """调试用：打印并返回原始 state（不改动内容）"""
    print("\n" + "="*30 + " LANGGRAPH GLOBAL STATE " + "="*30)
    # pprint 输出更易读
    pprint.pprint(state)
    print("="*90 + "\n")
    # 不修改 state 的语义，返回空 dict 表示没有新增字段，框架通常会合并
    # 也可以显式返回 state，但大多数 StateGraph 实现会合并节点返回值到全局 state
    return {
        "step": "debug"
    }
...
def create_search_assistant():
    ...
    workflow.add_edge("answer", "debug")
